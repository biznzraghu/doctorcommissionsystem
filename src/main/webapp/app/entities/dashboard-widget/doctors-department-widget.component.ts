...
@Component({
  selector: 'jhi-doctors-department-widget-graph',
  templateUrl: 'doctors-department-widget.component.html'
})
export class DoctorsDepartmentWidgetComponent implements OnInit, AfterViewInit {
  @Input() data: any;
  widgetData: any;
      ...
      this.dashboardWidgetService
        ...
        .subscribe(
          (res: HttpResponse<any>) => {
            this.collectionChartData = res.body;
            // this.loading = false;
            this.generateGraph();
  ...
  generateGraph() {
    ...
    const doctRevenue = this.collectionChartData.doctorRevenue;
    const keys = this.collectionChartData.key;
    const doctName = this.collectionChartData.doctorName;

    for (let index = 0; index < keys.length; index++) {
      let element: any;
      let doctRev: any;

      if (doctName[index].indexOf(',') > 0) {
        element = doctName[index].split(',');
        doctRev = doctRevenue[index].split(',');
      } else {
        element = [doctName[index]];
        doctRev = [doctRevenue[index]];
      }
      if (element[0] !== ' ' && doctRev[0] >= 1) {
        element.forEach(function(obj, ind) {
          const key = obj;
          const value = doctRev[ind];
          valueData[index] = Number(value);
          if (key !== '' && value !== '') {
    ...
    const chartConfig = {
      bindto: this.elementRef.nativeElement.querySelector('#doctorWiseRevenueTrend'),
      size: {
        height: 340,
        width: 580
      },
      ...
      axis: {
        ...
        y: {
          ...
          tick: {
            format: (d: any) => '' + d.toFixed(2)
          }
        }
      }
    };

    if (this.collectionChartData) {
