...
@Component({
  ...
  template: `
    <div class="alerts" role="alert">
      <div *ngFor="let alert of alerts" [ngClass]="{ 'alert.position': true, toast: true }">
...
export class JhiAlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: JhiAlertService, private eventManager: JhiEventManager, private translateService: TranslateService) {
    ...
    this.cleanHttpErrorListener = eventManager.subscribe('gatewayApp.httpError', response => {
      ...
      let entityKey = null;
      const arr = httpResponse && httpResponse.headers && httpResponse.headers._headers ? Array.from(httpResponse.headers._headers) : [];
      const headers = [];
