...
export class AccountService {
  ...
  public checkHasAuthority(authorities: string[] | string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      if (!Array.isArray(authorities)) {
        authorities = [authorities];
      }
      const adminAuthorities = ['ROLE_ADMIN', '000000000'];
      const modifyPrivileges = [...authorities, ...adminAuthorities];
      const hasAnyAuthority = this.hasAnyAuthority(modifyPrivileges);
      if (hasAnyAuthority) {
        resolve(true);
      } else {
        reject(false);
