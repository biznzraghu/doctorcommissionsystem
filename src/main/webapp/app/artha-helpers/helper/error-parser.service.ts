...
export class ErrorParser {
  ...
  alertWithParams(error, params) {
    ...
    if (error.errorMessages && error.errorMessages.length > 0) {
      ...
      const msg = error.detail ? error.detail : error.title ? error.title : '';
      ...
      this.alertService.error(
        'global.messages.response-msg',
        error.detail ? { msg: error.detail } : error.title ? error.title : { msg: 'Oops!!! Something went wrong' },
        null
      );
      ...
      this.alertService.error(
        'global.messages.response-msg',
        error.description ? { msg: error.description } : error.params ? error.params : { msg: 'Oops!!! Something went wrong' },
        null
      );
    }
  }
