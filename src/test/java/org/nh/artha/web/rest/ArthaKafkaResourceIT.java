...
import org.nh.artha.config.KafkaProperties;
...
class ArthaKafkaResourceIT {
    ...
    void setup() {
        ...
        ArthaKafkaResource kafkaResource = new ArthaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/artha-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/artha-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
